    ...
    plugin: {
        ...
        tableCollapse: function(conf){
            ...
            table.on( 'responsive-resize', function ( e, datatable, columns ) {
                ...
                isDisplayDetailsControl({
                    target: $(this),
                    hiddenCount: count
                });
            });

            function isDisplayDetailsControl(conf){
                var target = conf.target,
                    hiddenCount = conf.hiddenCount;

                target.find('.details-control').remove();

                if( hiddenCount > 0 ){
                    target.find('thead > tr').prepend('<th class="details-control sorting_disabled"></th>');
                    target.find('tbody > tr:not(.child)').prepend('<td class=" details-control"></td>');
                }
            }; //end isDisplayDetailsControl function

            isDisplayDetailsControl({
                target: target,
                hiddenCount: target.find('thead th:not(:visible)').length
            });
        ...
        blockEqualHeight: function(){
            ...
            $('.ga-height-eq-parent').each(function(){
                ...
                parent_ele.find('.ga-height-eq-child').each(function(){
                    var this_height = $(this).height('auto').height(); //$(this).outerHeight()
                    if( this_height > maxHeight ) maxHeight = this_height;
                });

                var window_w = $(window).width();
                if(window_w > 768) parent_ele.find('.ga-height-eq-child').css( 'min-height', maxHeight );
